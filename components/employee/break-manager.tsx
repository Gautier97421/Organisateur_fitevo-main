                  ...
                  <DialogContent className="sm:max-w-md bg-white">
                    ...
                    <DialogFooter className="flex flex-col sm:flex-row gap-2 sm:gap-3">
                      <DialogTrigger asChild>
                        <Button
                          variant="outline"
                          className="w-full sm:w-auto text-sm md:text-lg px-4 md:px-6 border border-gray-300 hover:bg-gray-50 bg-white flex items-center justify-center gap-2"
                        >
                      ...
                      <Button
                        ...
                        className="w-full sm:w-auto bg-red-600 hover:bg-red-700 text-sm md:text-lg px-4 md:px-6 flex items-center justify-center gap-2"
                      >
                    ...
                    <DialogDescription className="text-sm md:text-base text-gray-600 leading-relaxed">
                      {isCurrentShortBreakComplete ? (
                        <span className="text-green-600 flex items-start gap-2">
                          <CheckCircle className="h-5 w-5" /> Votre pause de{' '}
                          {REQUIRED_BREAK_MINUTES} minutes est terminée !
                        </span>
                      ) : (
                        <>
                          Vous avez pris {shortBreakTotal} minutes sur les {REQUIRED_BREAK_MINUTES}{' '}
                          requises pour cette pause.
                          <br />
      ...
      {hasShortBreakInProgress ? (
        ...
        <div className="space-y-2 w-full">
          <Button
            ...
            size="sm"
            className="bg-red-600 hover:bg-red-700 text-white shadow-lg text-sm md:text-base w-full sm:w-auto flex items-center justify-center gap-2"
          >
            <Coffee className="h-4 w-4" /> Reprendre la pause{' '}
            {isJournee ? `${currentShortBreakIndex}/${maxShortBreaks}` : ''}
          </Button>
          <p className="text-xs text-red-600 text-center flex items-center justify-center gap-1">
            <Clock className="h-3 w-3" /> {remainingBreakTime} min restantes ({shortBreakProgress}{' '}
            prises)
          </p>
          {isJournee && !lunchBreakTaken && (
            <Button
              ...
              variant="outline"
              size="sm"
              className="border-2 border-red-300 hover:bg-red-50 bg-white text-sm md:text-base w-full sm:w-auto flex items-center justify-center gap-2"
            >
        ...
        <div className="space-y-2 w-full">
          <Button
            disabled
            size="sm"
            className="bg-green-600 text-white shadow-lg text-sm md:text-base w-full sm:w-auto flex items-center justify-center gap-2"
          >
            ...
            <DialogContent className="sm:max-w-md bg-white">
              <DialogHeader>
                ...
                <DialogDescription className="text-sm md:text-base text-gray-600 leading-relaxed">
                  ...
                  <br />
                  <strong>
                    Vous pourrez reprendre le travail à tout moment et continuer votre pause plus
                    tard.
                  </strong>
                </DialogDescription>
              </DialogHeader>
          ...
          {isJournee && (
            <Dialog open={showLunchDialog} onOpenChange={setShowLunchDialog}>
              <DialogTrigger asChild>
                ...
                >
                  <Coffee className="h-4 w-4" />{' '}
                  {lunchBreakTaken ? 'Pause midi déjà prise' : 'Prendre la pause midi'}
                </Button>
              </DialogTrigger>
              <DialogContent className="sm:max-w-md bg-white">
                <DialogHeader>
                  ...
                  <DialogDescription className="text-sm md:text-base text-gray-600 leading-relaxed">
                    Cette pause midi est libre, mais elle ne peut être prise qu'une seule fois
                    pendant la journée.
                  </DialogDescription>
                </DialogHeader>
                <DialogFooter className="flex flex-col sm:flex-row gap-2 sm:gap-3">
