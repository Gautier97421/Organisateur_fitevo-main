...
export function WorkScheduleCalendar({ hasWorkScheduleAccess = true }: WorkScheduleCalendarProps) {
  ...
  const loadUserGyms = async () => {
    try {
      ...
      const userGymData = Array.isArray(result.data)
        ? result.data
        : result.data
          ? [result.data]
          : []
  ...
  const loadSchedules = async (gyms?: any[]) => {
    try {
      ...
      const result = await response.json()
      let schedulesData = Array.isArray(result.data)
        ? result.data
        : result.data
          ? [result.data]
          : []
      ...
      schedulesData = schedulesData.filter(
        (s: WorkSchedule) => s.gym_id && gymIds.includes(s.gym_id),
      )
      ...
      schedulesData = schedulesData.filter((s: any) => !s.is_temporary)
  ...
  const loadEmployees = async (gyms?: any[]) => {
    try {
      ...
      const result = await response.json()
      const employeesData = Array.isArray(result.data)
        ? result.data
        : result.data
          ? [result.data]
          : []
      ...
      if (userGymsResponse.ok) {
        ...
        if (usersResponse.ok) {
          ...
          const filteredEmployees = employeesData.filter((emp: any) => {
            ...
            const userId = [...userIdToEmail.entries()].find(
              ([id, email]) => email === emp.email,
            )?.[0]
  ...
  const addSchedule = async () => {
    ...
    try {
      ...
      if (checkResponse.ok) {
        ...
        console.log(
          `[Employee] Vérification pour employé: ${selectedEmployee.email} le ${selectedDateStr}`,
        )
    ...
    <div className="space-y-4 md:space-y-6">
        ...
        <Card className="border border-gray-200 shadow-lg bg-white">
          <CardContent className="p-4">
            <div className="flex items-center gap-3">
              <label className="text-sm font-medium text-gray-700">Filtrer par salle :</label>
        ...
        <CardContent className="p-2 md:p-6">
          ...
          <div className="grid grid-cols-7 gap-1 md:gap-2 mb-2 md:mb-4">
            {dayNames.map((day) => (
              <div
                key={day}
                className="text-center font-semibold text-gray-600 py-1 md:py-2 text-xs md:text-sm"
              >
                {day}
              </div>
            ))}
      ...
      <Card className="border border-gray-200 shadow-xl bg-white">
        <CardContent className="p-3 md:p-4">
          <h4 className="font-semibold mb-2 md:mb-3 text-sm md:text-base text-gray-900">
            Légende des employés :
          </h4>
        ...
        <DialogContent className="max-w-[90vw] sm:max-w-md bg-white max-h-[90vh] overflow-y-auto rounded-2xl">
          ...
          <div className="space-y-4">
            <div>
              ...
              >
                <SelectTrigger
                  className={`border-2 rounded-xl bg-white text-gray-900 ${attemptedSubmit && !selectedEmployee ? 'border-red-500' : ''}`}
                >
            ...
            {userGyms.length > 1 && (
              <div>
                ...
                <Select value={selectedGymId} onValueChange={setSelectedGymId}>
                  <SelectTrigger
                    className={`border-2 rounded-xl bg-white text-gray-900 ${attemptedSubmit && selectedGymId === 'all' ? 'border-red-500' : ''}`}
                  >
                        ...
                        >
                          <CardContent className="p-4">
                            <div className="flex items-start justify-between mb-2">
                              <div className="flex items-center space-x-2">
                                <div
                                  className={`w-4 h-4 ${getEmployeeColor(schedule.employee_email)} rounded`}
                                ></div>
                            ...
                            <div className="space-y-1 text-sm text-gray-600">
                              ...
                              {schedule.break_start_time && schedule.break_duration && (
                                <p className="text-gray-600 text-xs mt-1">
                                  🕐 Pause : {schedule.break_start_time} ({schedule.break_duration}{' '}
                                  min)
                                </p>
                              )}
                            </div>
        ...
        <DialogContent className="max-w-[90vw] sm:max-w-md bg-white max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            ...
            <DialogDescription className="text-sm md:text-base text-gray-600">
              {scheduleToEdit && (
                <>
                  {scheduleToEdit.employee_name} -{' '}
                  {new Date(scheduleToEdit.work_date + 'T00:00:00').toLocaleDateString('fr-FR', {
                    weekday: 'long',
                    day: 'numeric',
                    month: 'long',
                    year: 'numeric',
              ...
              {scheduleToDelete && (
                <div className="mt-3 p-3 bg-gray-50 rounded-lg">
                  ...
                  <p className="text-sm text-gray-600">
                    {new Date(scheduleToDelete.work_date + 'T00:00:00').toLocaleDateString('fr-FR')}{' '}
                    - {scheduleToDelete.start_time} à {scheduleToDelete.end_time}
                  </p>
                </div>
              )}
            </DialogDescription>
