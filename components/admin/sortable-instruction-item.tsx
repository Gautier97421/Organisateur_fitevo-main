  ...
  return (
    <Card
      ...
      className={`border-2 shadow-sm ${
        item.is_active ? "border-blue-200 bg-white" : "border-gray-200 bg-gray-50 opacity-70"
      }`}
    >
        ...
        {editingId === item.id ? (
          ...
          <div className="space-y-2 md:space-y-3">
            <Input
              ...
              className="border-gray-300 focus:border-blue-500 font-medium text-sm md:text-base"
            />
            <Textarea
              value={editDescription}
              onChange={(e) => setEditDescription(e.target.value)}
              placeholder="Description..."
              className="border-gray-300 focus:border-blue-500 min-h-[50px] md:min-h-[60px] text-sm md:text-base"
            />
            <div className="flex flex-col sm:flex-row gap-2">
              <Button
                onClick={() => onSaveEdit(item.id)}
                disabled={isSaving}
                className="bg-blue-600 hover:bg-blue-700 text-white flex-1 text-sm md:text-base"
                size="sm"
              >
          ...
          <div className="flex flex-col sm:flex-row items-start gap-2 sm:gap-3">
            <div className="flex items-center gap-2">
              ...
              >
                <GripVertical className="w-4 h-4 md:w-5 md:h-5 text-gray-400 hover:text-blue-600" />
              </div>
              <div className="w-6 h-6 md:w-8 md:h-8 rounded-full bg-blue-500 text-white flex items-center justify-center text-xs md:text-sm font-bold flex-shrink-0 shadow-sm">
                {index + 1}
              </div>
            </div>
            ...
            <div className="flex items-center gap-1 sm:gap-2 w-full sm:w-auto justify-end sm:justify-start">
              ...
              <Button
                ...
                variant="ghost"
                size="sm"
                className="text-blue-600 hover:text-blue-700 text-xs md:text-sm px-2"
              >
                Modifier
              </Button>
              <Button
