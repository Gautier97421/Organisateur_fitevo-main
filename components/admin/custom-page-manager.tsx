    ...
    <Card className="border border-gray-200 bg-white">
      <CardHeader className="border-b border-gray-200 bg-gray-50 p-4 md:p-6">
        ...
        <p className="text-sm text-gray-600 mt-2">
          Créez des pages personnalisées qui apparaîtront comme des onglets dans le panneau
          d'administration. Les admins pourront ensuite les utiliser pour gérer diverses procédures.
        </p>
      </CardHeader>
      <CardContent className="p-3 md:p-6 space-y-3 md:space-y-4">
            ...
            <CardContent className="p-3 md:p-4 space-y-3">
              ...
              <div className="space-y-2">
                ...
                <p className="text-xs text-gray-500 mb-2">
                  Sélectionnez les rôles qui peuvent voir cette page. Laissez vide pour que tous les
                  employés y aient accès.
                </p>
          ...
          {!Array.isArray(pages) || pages.length === 0 ? (
            ...
            pages.map((page) => {
              ...
              return (
                <Card
                  key={page.id}
                  className={`border ${page.isActive ? 'border-gray-200' : 'border-gray-300 bg-gray-100 opacity-60'}`}
                >
                  <CardContent className="p-4">
                    <div className="flex items-start justify-between gap-3">
                      <div className="flex items-start gap-3 flex-1 min-w-0">
                        {IconComponent && (
                          <IconComponent
                            className={`w-5 h-5 flex-shrink-0 mt-0.5 ${page.isActive ? 'text-red-600' : 'text-gray-400'}`}
                          />
                        )}
                        <div className="flex-1 min-w-0">
                          <h3
                            className={`font-semibold ${page.isActive ? 'text-gray-900' : 'text-gray-500'}`}
                          >
                            {page.title}
                            {!page.isActive && (
                            ...
                            {pageRoleIds.length === 0 ? (
                              ...
                              pageRoleIds.map((roleId) => {
                                ...
                                return (
                                  <span
                                    key={roleId}
                                    className={`px-2 py-1 rounded ${getRoleColor(role.color)}`}
                                  >
                                    {role.name}
                                  </span>
                                )
        ...
        >
          <AlertDialogContent>
            <AlertDialogHeader>
              ...
              <AlertDialogDescription>
                Êtes-vous sûr de vouloir supprimer cette page ? Tous ses éléments seront également
        ...
        <AlertDialog
          open={showRolesDialog !== null}
          onOpenChange={(open) => !open && setShowRolesDialog(null)}
        >
          <AlertDialogContent className="max-w-md">
            <AlertDialogHeader>
              ...
              <AlertDialogDescription>
                Sélectionnez les rôles qui peuvent voir cette page. Si aucun rôle n'est sélectionné,
                tous les employés y auront accès.
              </AlertDialogDescription>
            </AlertDialogHeader>
            <div className="py-4">
              {(() => {
                  ...
                  <div className="space-y-3">
                    <div className="flex flex-wrap gap-2 p-4 bg-gray-50 rounded-lg border border-gray-200 min-h-[80px]">
                      {roles.length === 0 ? (
                        <span className="text-sm text-gray-500 italic">
                          Aucun rôle créé. Créez des rôles dans l'onglet "Rôles".
                        </span>
                      ) : (
                        roles.map((role) => {
                    ...
                    {currentRoleIds.length === 0 ? (
                      ...
                      <p className="text-sm text-purple-600 flex items-center gap-1">
                        <Shield className="w-4 h-4" />
                        Accessible uniquement aux {currentRoleIds.length} rôle
                        {currentRoleIds.length > 1 ? 's' : ''} sélectionné
                        {currentRoleIds.length > 1 ? 's' : ''}
                      </p>
                    )}
                  </div>
                )
